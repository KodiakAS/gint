...
#if GINT_BENCH_BITS == 256 && !defined(GINT_ENABLE_CH_COMPARE) && !defined(GINT_ENABLE_BOOST_COMPARE)
namespace prepared_divisor_bench
{
template <typename Int>
    ...
    for (auto _ : state)
    {
        if (Setup)
        {
            ...
            for (unsigned j = 0; j < Batch; ++j)
                consume<Round>(current, data[index++ & (kDataN - 1)], divisor);
        }
        else
        {
            for (unsigned j = 0; j < Batch; ++j)
                consume<Round>(prepared, data[index++ & (kDataN - 1)], divisor);
        }
    }
...
void add(const char * kind, const char * operation, unsigned digits, unsigned scale, unsigned sign)
{
    ...
    const std::string label = std::string("PreparedDivisor/") + kind + "/" + operation + "/P" + std::to_string(digits) + "/S"
        + std::to_string(scale) + "/" + signs[sign] + "/gint";
...
void registerCases(bool full_matrix)
{
    for (unsigned digits : {18u, 38u, 65u})
        for (unsigned scale : {0u, 2u, 19u, 20u, 30u, 38u})
        {
            ...
            {
                if (!full_matrix && !(digits == 65 && scale == 30 && sign == 2))
                    continue;
                if (sign == 3)
                {
                    addOperations<gint::UInt256, OrdinaryDivisor<gint::UInt256>>("Ordinary", digits, scale, sign);
                    addOperations<gint::UInt256, gint::prepared_divisor<gint::UInt256>>("Prepared", digits, scale, sign);
                }
                else
                {
                    addOperations<gint::Int256, OrdinaryDivisor<gint::Int256>>("Ordinary", digits, scale, sign);
                    addOperations<gint::Int256, gint::prepared_divisor<gint::Int256>>("Prepared", digits, scale, sign);
                }
            }
        }
}
...
#if GINT_BENCH_BITS == 256 && !defined(GINT_ENABLE_CH_COMPARE) && !defined(GINT_ENABLE_BOOST_COMPARE)
    prepared_divisor_bench::registerCases(full_matrix);
#endif
    // Addition
