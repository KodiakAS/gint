...
- `gint::prepared_divisor<T>` 仅定义 `T = Int256` 与 `T = UInt256` 两种形式。
  显式构造函数复制并保存同型除数，构造不抛异常；const 成员函数 `divmod(a)` 接受同型
  被除数并返回 `divmod_result<T>`，结果及除零行为与 `gint::divmod(a, d)` 相同。
  保存的状态可以复制并用于不同被除数；零除数的 checked 异常发生在运算时。
