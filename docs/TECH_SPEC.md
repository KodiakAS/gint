...
- 内建整数、`__int128` 与 `unsigned __int128` 可以隐式构造任意受支持的
  `integer<Bits, Signed>`；扩大位宽时按源类型符号扩展或零扩展，缩小时截断超出
  目标 `Bits` 的高位。
- 不同 `integer` 类型之间的构造是显式的；跨位宽或 signedness 赋值受支持。扩大
  位宽时按源 `Signed` 与符号位扩展，缩小时保留目标位宽的低位。
- 转换到内建整数时保留目标宽度的低位；有符号目标按补码解释这些位。
- 转换到 `bool` 为显式转换，非零值为 `true`。
- 从内建整数构造在 C++11 下为 `constexpr`；跨 `integer` 类型的构造，以及
  copy/move、内建整数或跨 `integer` 类型的赋值从 C++14 起为 `constexpr`。浮点与
  字符串的构造和赋值不属于上述保证。
