...
- 除零检查（可选）：
  ...
  - 宏已定义（需在包含头文件前定义，且同一翻译单元内生效）：除零或模零抛出 `std::domain_error`。作为头文件模板库，建议同一 target 统一配置该宏。
...
- `GINT_ENABLE_DIVZERO_CHECKS`：在当前翻译单元内启用除零/模零抛出 `std::domain_error` 的检查；应在包含 `gint/gint.h` 前定义。
