...
- 位移量为负：保持不变（no-op，调试构建下会触发 `assert` 提示）。
  - 左移丢弃高位，按模 `2^Bits` 包裹。
...
- 位移量为负：结果保持不变（不触发 UB；调试构建下会 `assert` 提示）。
...
- 乘法（mul）：
  ...
  - 256 位：采用 Comba 风格展开（对角线累加 + 显式进位），更利于编译器优化寄存器与减少中间存取。
  - 其它位宽：一般 O(n^2) 学校乘法。
- 除法/取模（div/mod）：
...
- 单 limb 乘法（mul_limb）：
  - 移除 `mul_limb<4>` 特化，统一使用通用版本（逐 limb 乘加 + 进位累加），简化实现，便于编译器做向量化/调度。
