...
通用 Knuth 内核；移位由上层检查位数并选择扩展 fill，下层执行数组移动。
左移内核接收完整位数并分解 limb/bit 偏移，保留两者的计算关系；右移接收已
分解的偏移及 fill。
...
完整初始化；小除法用数组引用保留编译期长度，避免提取后丢失数组形状信息。
原地移位和位运算使用明确的 assign 接口；移位内核接收非零且范围内的偏移，
独立输出的移位不允许
...
AArch64 GCC 的 256 位原生整数取模直接使用既有余数内核，避免提取后仍保留
未使用商的计算；符号适配仍在整数层，其余位宽和目标继续使用原有分派。
