...
`prepared_divisor<Int256/UInt256>` 为固定除数保存一个独立对象，不改变整数对象布局。
调用方必须显式构造并复用该对象，通过成员 `divmod(value)` 使用缓存；普通 `/`、
`%` 和自由函数 `gint::divmod(a, d)` 不会创建或调用它。仅升级头文件不会让已有
调用获得缓存收益。
只有非 2 的幂、绝对值恰好占两个 limb 的除数预先计算归一化移位、归一化除数和
128-bit 倒数；每次运算仍要归一化被除数。其他除数形态调用普通 `divmod`。
x86-64 GCC 8 强制内联外层入口，并让绝对值小于或等于除数的短被除数直接形成
商余结果；其他组合保留普通内联策略和内核的短输入处理。内核复用两 limb 估商和
校正步骤，最后恢复符号。缓存入口使用独立内核，保持普通除法内核的调用边界。

缓存的性能收益依赖于输入形态和复用次数。调用方应在表达式或批次边界按固定除数
与工作量选择是否使用，不能将每行构造、任意除数的调用一律替换为缓存接口。
测量时需计入构造成本，并与同一构建的普通 `divmod` 对比。

```cpp
gint::prepared_divisor<gint::Int256> fixed_divisor(divisor);
for (const auto & value : values) {
    const auto result = fixed_divisor.divmod(value);
    consume(result.quotient, result.remainder);
}
```

固定十进制 scale 是一种用途；接口不限定除数必须是十进制幂。
